...
load('JpegCoeff.mat');
load('scan_order.mat');
load('jpegcodes_space_hide.mat');
load('hall.mat');
load('message.mat');
load('C_matrix_after_DCnQuntize_space_field.mat');%用于比较
...
clear i_x i_y C_matrix_rcv C_matrix_iQuantize hall_color
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%空域评价抗编码能力%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% back=double(P_matrix);
% message_recover=zeros(120,168);
% for i=1:120
%     for j=1:168
%         if(mod(back(i,j),2)==0) %偶数提取0
%             message_recover(i,j)=0;
%         else                    %奇数提取1
%             message_recover(i,j)=1;
%         end
%     end
% end
% diff=0;
% for i=1:120
%     for j=1:168
%         diff=diff+double(message(i,j)~=message_recover(i,j));
%     end
% end
% clear ij
